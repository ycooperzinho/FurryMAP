...
db.serialize(() => {
  ...
  [
    ["users", "email", "TEXT"], ["users", "displayName", "TEXT"], ["users", "telegram", "TEXT"],
    ["users", "discord", "TEXT"], ["users", "city", "TEXT"], ["users", "phone", "TEXT"],
    ["users", "status", "TEXT"], ["users", "about", "TEXT"], ["users", "photo", "TEXT"],
    ["users", "isAdmin", "INTEGER DEFAULT 0"], ["users", "createdAt", "TEXT DEFAULT CURRENT_TIMESTAMP"],
    ["markers", "createdAt", "TEXT DEFAULT CURRENT_TIMESTAMP"], ["markers", "updatedAt", "TEXT DEFAULT CURRENT_TIMESTAMP"],
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!file.mimetype || !file.mimetype.startsWith("image/")) return cb(new Error("Envie apenas imagem."));
    cb(null, true);
  },
});
...
function requireLogin(req, res, next) {
  if (!req.session.userId) return res.status(401).json({ error: "Você precisa estar logado." });
  next();
}
function requireAdmin(req, res, next) {
  ...
  db.get("SELECT isAdmin FROM users WHERE id = ?", [req.session.userId], (err, user) => {
    ...
    if (!user || user.isAdmin !== 1) return res.status(403).json({ error: "Acesso negado." });
    next();
  });
}
...
app.post("/api/register", (req, res) => {
  ...
  if (!email || !username || !password) return res.status(400).json({ error: "Preencha Gmail, usuário e senha." });
  if (!email.endsWith("@gmail.com")) return res.status(400).json({ error: "Use um Gmail válido terminando em @gmail.com." });
  ...
  db.get("SELECT id FROM users WHERE username = ? OR email = ?", [username, email], async (err, existing) => {
    ...
    db.get("SELECT COUNT(*) AS total FROM users", [], (countErr, row) => {
      ...
      db.run("INSERT INTO users (email, username, password, displayName, isAdmin) VALUES (?, ?, ?, ?, ?)", [email, username, hash, username, isAdmin], function (insertErr) {
        if (insertErr) return res.status(500).json({ error: "Erro ao criar conta." });
        req.session.userId = this.lastID;
        res.json({ message: isAdmin ? "Conta criada. Você é o admin principal." : "Conta criada.", user: { id: this.lastID, username, email, isAdmin: !!isAdmin } });
      });
    });
  });
});
app.post("/api/login", (req, res) => {
  ...
  const password = clean(req.body.password);
  if (!login || !password) return res.status(400).json({ error: "Preencha usuário/Gmail e senha." });
  db.get("SELECT * FROM users WHERE username = ? OR email = ?", [login, login.toLowerCase()], async (err, user) => {
    ...
    res.json({ message: "Login realizado.", user: publicUser(user) });
  });
});
app.post("/api/logout", (req, res) => req.session.destroy(() => res.json({ message: "Você saiu." })));
app.put("/api/profile", requireLogin, (req, res) => {
  ...
  db.run("UPDATE users SET displayName=?, telegram=?, discord=?, city=?, phone=?, status=?, about=? WHERE id=?", [...fields, req.session.userId], (err) => {
...
function saveUploadedPhoto(req, res) {
  ...
  db.run("UPDATE users SET photo = ? WHERE id = ?", [photo, req.session.userId], (err) => {
    if (err) return res.status(500).json({ error: "Erro ao salvar foto." });
    res.json({ message: "Foto enviada.", photo });
  });
}
...
app.get("/api/users/:id", (req, res) => {
  db.get(`SELECT users.*, markers.lat, markers.lng FROM users LEFT JOIN markers ON markers.userId = users.id WHERE users.id = ?`, [Number(req.params.id)], (err, user) => {
    if (err) return res.status(500).json({ error: "Erro no servidor." });
    if (!user) return res.status(404).json({ error: "Perfil não encontrado." });
    res.json({ user: publicUser(user), marker: user.lat != null && user.lng != null ? { lat: user.lat, lng: user.lng } : null });
  });
});
app.get("/api/markers", (_req, res) => {
  db.all(`SELECT markers.id AS markerId, markers.userId AS markerUserId, markers.lat, markers.lng, users.* FROM markers JOIN users ON users.id = markers.userId ORDER BY markers.updatedAt DESC`, [], (err, rows) => {
    if (err) return res.status(500).json({ error: "Erro ao buscar marcadores." });
    res.json({ markers: rows.map((r) => ({ id: r.markerId, lat: r.lat, lng: r.lng, user: publicUser(r) })) });
  });
});
app.post("/api/markers", requireLogin, (req, res) => {
  ...
  const safeLat =
    p.lat + (Math.random() - 0.5) * margem;

  const safeLng =
    p.lng + (Math.random() - 0.5) * margem;
...
app.get("/api/admin/users", requireAdmin, (_req, res) => {
  db.all(`SELECT users.*, markers.lat, markers.lng FROM users LEFT JOIN markers ON markers.userId = users.id ORDER BY users.id DESC`, [], (err, rows) => {
    if (err) return res.status(500).json({ error: "Erro ao buscar usuários." });
    res.json({ users: rows.map((u) => ({ ...publicUser(u), hasMarker: u.lat != null && u.lng != null })) });
  });
});
app.put("/api/admin/users/:id", requireAdmin, (req, res) => {
  const fields = ["displayName", "telegram", "discord", "city", "phone", "status", "about"].map((k) => clean(req.body[k]));
  const isAdmin = req.body.isAdmin ? 1 : 0;
  db.run("UPDATE users SET displayName=?, telegram=?, discord=?, city=?, phone=?, status=?, about=?, isAdmin=? WHERE id=?", [...fields, isAdmin, Number(req.params.id)], (err) => {
...
app.delete("/api/admin/users/:id", requireAdmin, (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run("DELETE FROM users WHERE id = ?", [id], (err) => {
      ...
      if (deletingSelf) return req.session.destroy(() => res.json({ message: "Sua própria conta admin foi deletada." }));
...
app.listen(PORT, "0.0.0.0", () => console.log(`FurMAP rodando na porta ${PORT}`));
